...
def _js_divergence(attention_map1, attention_map2):
    ...
    mid_attn = (attention_map1 + attention_map2) / 2
    
    p = dist.Categorical(probs=attention_map1)
    q = dist.Categorical(probs=attention_map2)
    m = dist.Categorical(probs=mid_attn)

    kl_divergence_pm = dist.kl_divergence(p, m)
    kl_divergence_qm = dist.kl_divergence(q, m)

    div = (kl_divergence_qm + kl_divergence_pm) / 2
    return div
...
def _asymmetric_kl(attention_map1, attention_map2):
    if len(attention_map1.shape) > 1:
        attention_map1 = attention_map1.reshape(-1)
    if len(attention_map2.shape) > 1:
        attention_map2 = attention_map2.reshape(-1)
    ...
    return kl_divergence_pq

def calculate_positive_loss(attention_maps, modifier, noun, symmetric=False, JS_div=False):
    src_indices = modifier
    dest_indices = noun

    kl_func = _symmetric_kl if symmetric else _asymmetric_kl

    if JS_div:
        kl_func = _js_divergence

    if isinstance(src_indices, list) and isinstance(dest_indices, list):
        wp_pos_loss = [
            kl_func(attention_maps[s], attention_maps[d])
            for (s, d) in itertools.product(src_indices, dest_indices)
        ]
        positive_loss = max(wp_pos_loss)
    elif isinstance(dest_indices, list):
        wp_pos_loss = [
            kl_func(attention_maps[src_indices], attention_maps[d])
            for d in dest_indices
        ]
        positive_loss = max(wp_pos_loss)
    elif isinstance(src_indices, list):
        wp_pos_loss = [
            kl_func(attention_maps[s], attention_maps[dest_indices])
            for s in src_indices
        ]
        positive_loss = max(wp_pos_loss)
    else:
        positive_loss = kl_func(
...
def _calculate_outside_loss(attention_maps, src_indices, outside_loss, JS_div=False):
    negative_loss = []
    computed_pairs = set()
    pair_counter = 0

    kl_func = _js_divergence if JS_div else _symmetric_kl
