...
from utils.attention_utils import AttendExciteAttnProcessor_custom
...
def register_cross_attention_hook(unet):
    for name, module in unet.named_modules():
        ...
        if isinstance(module.processor, AttnProcessor):
            ...
            module.processor.store_attn_map = True
        elif isinstance(module.processor, AttendExciteAttnProcessor_custom):
            module.processor.store_attn_map = True
...
def upscale(attn_map, target_size):
    ...
    if target_size[0]*target_size[1] != attn_map.shape[1]:
        ...
        t_shape = attn_map.shape[-1] ** 0.5
        t_shape = int(t_shape)


        attn_map = rearrange(attn_map, 'c (h w) -> c h w', h=t_shape, w=t_shape)
        # attn_map = attn_map.view(attn_map.shape[0], ) # (77, 32,32)
...
def get_net_attn_map(image_size, batch_size=2, instance_or_negative=False, detach=True):
    ...
    net_attn_maps = net_attn_maps.reshape(net_attn_maps.shape[0], target_size[0],target_size[0]) # (77,64*64) -> (77,64,64)
